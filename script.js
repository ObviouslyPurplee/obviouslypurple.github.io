...
:root[data-theme="dark"] .tab {
  background: transparent;
  color: #e9e6ea;
  border: 1px solid rgba(255, 155, 200, .35);
  transition: all .2s ease
}
:root[data-theme="dark"] .tab:hover {
    background:rgba(255,155,200,.25);
    color:#fff;
...
:root[data-theme="dark"] #fogFX {
  mix-blend-mode: screen;
  filter: blur(3px);
}
:root.shimmer-on #shimmerFX {
  opacity: .5;
}
:root.shimmer-on {
  --accent: #b79bff;
}
:root.galaxy-on #shimmerFX {
  opacity: .78;
}
:root.galaxy-on {
  --accent: #b79bff;
...
.shot.has-img::after {
  ...
  background: radial-gradient(80% 60% at 12% 12%, rgba(255, 155, 200, .22), rgba(255, 155, 200, 0) 60%), linear-gradient(to bottom, rgba(12, 10, 14, .22), rgba(12, 10, 14, 0) 55%);
}
.card:hover .shot.has-img::after {
  opacity: 1
}
.labels {
  position: absolute;
...
.see-more {
  ...
  overflow: hidden;
  color: var(--label-ink);
  border: 1px solid rgba(var(--ring),.7);
  background: linear-gradient(180deg, rgba(var(--t), var(--t-top-a)), rgba(var(--t), var(--t-bottom-a))) padding-box;
  backdrop-filter: blur(12px) saturate(160%);
  -webkit-backdrop-filter: blur(12px) saturate(160%);
  box-shadow: inset 0 1px 0 rgba(255,255,255,.55), inset 0 -12px 22px rgba(0,0,0,.22), 0 10px 28px rgba(0,0,0,.35);
...
.cl-header { display: flex; justify-content: space-between; align-items: center; padding: 14px 16px; border-bottom: 1px solid var(--pink) }
...
.project-detail-header { margin: 20px 0; }
.project-detail-content { display: grid; grid-template-columns: 1fr 1.5fr; gap: 24px; align-items: start; }
.project-detail-image img { width: 100%; border-radius: var(--radius); background: var(--card); }
.project-detail-text h2 { margin-top: 0; margin-bottom: 16px; font-size: 24px; color: var(--ink); }
.project-detail-text p { font-size: 14px; line-height: 1.8; color: var(--muted); margin: 0; }
@media (max-width: 768px) { .project-detail-content { grid-template-columns: 1fr; } }
