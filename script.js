...
(function() { if (window.__fogFX) return; window.__fogFX = true; const cvs = document.getElementById('fogFX'); if (!cvs) return; const ctx = cvs.getContext('2d'); let W = 0, H = 0, DPR = Math.max(1, Math.min(2, window.devicePixelRatio || 1)); const SCALE = 2; let last = 0; let blobs = []; const isDark = () => document.documentElement.getAttribute('data-theme') === 'dark' || window.matchMedia('(prefers-color-scheme: dark)').matches; const isReduced = () => document.documentElement.getAttribute('data-motion') === 'reduced'; function colors() { return isDark() ? ['rgba(255,190,220,0.16)', 'rgba(255,190,220,0)'] : ['rgba(255,216,234,0.50)', 'rgba(255,216,234,0)']; } function seed() { blobs.length = 0; const base = Math.max(10, Math.round((W * H) / 300000)); const n = Math.min(22, Math.max(10, Math.round(base * (isReduced() ? 0.7 : 1)))); for (let i = 0; i < n; i++) { const r = 160 + Math.random() * 320; const ang = Math.random() * Math.PI * 2; const spd = (isReduced() ? 0.002 : 0.004) + Math.random() * 0.002; blobs.push({ x: Math.random() * W, y: Math.random() * H, r, vx: Math.cos(ang) * spd, vy: Math.sin(ang) * spd, wob: Math.random() * Math.PI * 2, wobAmp: 0.2 + Math.random() * 0.4, wobSpd: 0.0003 + Math.random() * 0.0006 }); } } function resize() { W = innerWidth; H = innerHeight; cvs.style.width = W + 'px'; cvs.style.height = H + 'px'; cvs.width = Math.ceil(W / SCALE) * DPR; cvs.height = Math.ceil(H / SCALE) * DPR; ctx.setTransform(DPR, 0, 0, DPR, 0, 0); ctx.imageSmoothingEnabled = false; seed(); } function draw(dt) { ctx.clearRect(0, 0, cvs.width, cvs.height); const [c0, c1] = colors(); for (const b of blobs) { const m = isReduced() ? 0.5 : 1; b.wob += b.wobSpd * dt * m; b.x += b.vx * dt * m + Math.cos(b.wob) * 0.02 * dt; b.y += b.vy * dt * m + Math.sin(b.wob) * 0.02 * dt; const pad = b.r + 40; if (b.x < -pad) b.x = W + pad; if (b.x > W + pad) b.x = -pad; if (b.y < -pad) b.y = H + pad; if (b.y > H + pad) b.y = -pad; const cx = b.x / SCALE, cy = b.y / SCALE, rr = b.r / SCALE; const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, rr); g.addColorStop(0, c0); g.addColorStop(1, c1); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(cx, cy, rr, 0, Math.PI * 2); ctx.fill(); } } function loop(ts) { const dt = Math.min(50, (ts - last) || 16); last = ts; draw(dt); requestAnimationFrame(loop); } window.addEventListener('resize', resize, { passive: true }); document.addEventListener('theme:changed', resize); document.addEventListener('motion:changed', resize); resize(); requestAnimationFrame(loop); })();
(function() { if (window.__shimmerFX3) return; window.__shimmerFX3 = true; const cvs = document.getElementById('shimmerFX'); if (!cvs) return; const ctx = cvs.getContext('2d'); let W = 0, H = 0, DPR = Math.max(1, Math.min(2, window.devicePixelRatio || 1)); const SCALE = 2; let t = 0; let stars = []; let glitchT = 0; let comets = []; let cometT = 0; const isReduced = () => document.documentElement.getAttribute('data-motion') === 'reduced'; function resize() { W = innerWidth; H = innerHeight; cvs.style.width = W + 'px'; cvs.style.height = H + 'px'; cvs.width = Math.ceil(W / SCALE) * DPR; cvs.height = Math.ceil(H / SCALE) * DPR; ctx.setTransform(DPR, 0, 0, DPR, 0, 0); ctx.imageSmoothingEnabled = false; seedStars(); } function seedStars() { const base = Math.max(120, Math.round((W * H) / 18000)); const count = Math.round(base * (isReduced() ? 0.5 : 1)); stars = Array.from({ length: count }, () => ({ x: Math.random() * W, y: Math.random() * H, tw: Math.random() * Math.PI * 2, s: (Math.random() * 2 + 0.6), hue: Math.random() < 0.18 ? 'orange' : 'blue' })); } function drawNebula() { const cols = Math.ceil(W / (SCALE * 7)), rows = Math.ceil(H / (SCALE * 7)); for (let yy = 0; yy < rows; yy++) { for (let xx = 0; xx < cols; xx++) { const wv = Math.sin((xx + yy + t) * 0.14) * 0.5 + Math.sin((xx * 0.34 - yy * 0.22 + t * 0.9)) * 0.5; const a = (isReduced() ? 0.08 : 0.10) + (wv + 1) * (isReduced() ? 0.07 : 0.10); const r = 170 + Math.floor((wv + 1) * 60); const g = 140 + Math.floor((1 - wv) * 50); const b = 255; ctx.fillStyle = `rgba(${r},${g},${b},${a})`; ctx.fillRect(xx * 7, yy * 7, 7, 7); } } } function drawStars() { for (const s of stars) { s.tw += isReduced() ? 0.05 : 0.09; const a = (isReduced() ? 0.4 : 0.5) + (Math.sin(s.tw) * (isReduced() ? 0.25 : 0.4) + 0.3); ctx.fillStyle = s.hue === 'orange' ? `rgba(255,180,100,${a*0.9})` : `rgba(170,160,255,${a})`; const size = s.s; ctx.fillRect(Math.round(s.x / SCALE), Math.round(s.y / SCALE), size, size); } } function spawnComet() { if (isReduced()) return; const fromLeft = Math.random() < 0.5; const y = Math.random() * H * 0.9; const x = fromLeft ? -50 : W + 50; const vx = (fromLeft ? 1 : -1) * (2 + Math.random() * 3); const vy = -0.4 + Math.random() * 0.8; comets.push({ x, y, vx, vy, life: 220 }); } function drawComets() { for (let i = comets.length - 1; i >= 0; i--) { const c = comets[i]; c.x += c.vx; c.y += c.vy; c.life -= 1; for (let tix = 0; tix < 6; tix++) { const trailX = c.x - c.vx * tix * 3; const trailY = c.y - c.vy * tix * 3; const alpha = 0.18 - tix * 0.025; if (alpha <= 0) continue; ctx.fillStyle = `rgba(${tix%2?200:150},${tix%2?120:180},255,${alpha})`; ctx.fillRect(Math.round(trailX / SCALE), Math.round(trailY / SCALE), 3, 2); } ctx.fillStyle = 'rgba(255,170,80,0.9)'; ctx.fillRect(Math.round(c.x / SCALE), Math.round(c.y / SCALE), 2, 2); if (c.life <= 0 || c.x < -100 || c.x > W + 100 || c.y < -100 || c.y > H + 100) comets.splice(i, 1); } } function drawOrangeGlitch(dt) { if (isReduced()) return; glitchT -= dt; if (glitchT > 0) return; glitchT = 900 + Math.random() * 1200; const y = (Math.random() * H) | 0, h = 6 + ((Math.random() * 20) | 0), shift = (Math.random() > 0.5 ? 1 : -1) * (8 + ((Math.random() * 24) | 0)); ctx.save(); ctx.globalAlpha = 0.2; ctx.fillStyle = 'rgba(255,150,70,1)'; ctx.fillRect(shift, y, W, h); ctx.restore(); ctx.save(); ctx.globalAlpha = 0.12; ctx.fillStyle = 'rgba(120,180,255,1)'; ctx.fillRect(shift * 0.5, y + 2, W, h - 2); ctx.restore(); } function drawShimmer() { const cols = Math.ceil(W / (SCALE * 8)), rows = Math.ceil(H / (SCALE * 8)); for (let y = 0; y < rows; y++) { for (let x = 0; x < cols; x++) { const wv = Math.sin((x + y + t) * 0.14) * 0.5 + Math.sin((x * 0.32 - y * 0.24 + t * 0.9)) * 0.5; const a = (isReduced() ? 0.08 : 0.10) + (wv + 1) * (isReduced() ? 0.07 : 0.10); const r = 190 + Math.floor((wv + 1) * 35); const g = 160 + Math.floor((1 - wv) * 30); const b = 255; ctx.fillStyle = `rgba(${r},${g},${b},${a})`; ctx.fillRect(x * 8, y * 8, 8, 8); } } } function loop() { t += isReduced() ? 0.4 : 0.8; ctx.clearRect(0, 0, cvs.width, cvs.height); const root = document.documentElement; if (root.classList.contains('galaxy-on')) { drawNebula(); drawStars(); drawComets(); drawOrangeGlitch(16); if (!isReduced()) { cometT -= 16; if (cometT <= 0) { cometT = 1500 + Math.random() * 2000; spawnComet(); } } } else if (root.classList.contains('shimmer-on')) { drawShimmer(); } requestAnimationFrame(loop); } window.addEventListener('resize', resize, { passive: true }); document.addEventListener('motion:changed', () => { resize(); }); resize(); loop(); })();
